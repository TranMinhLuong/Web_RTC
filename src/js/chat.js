...
function sendMesss(value, socketId_user, socket) {
    ...
    if (!checkUserChat('Tôi'))
        layout_chat.innerHTML += ' <span class="layout-chat-username me-mess">Tôi</span>: <span class="layout-chat-time" id="time">(' + time + ')</span><div class="layout-chat-value">' + value + '</div>'
    else
        layout_chat.innerHTML += ' <div class="layout-chat-value">' + value + '</div>'
}
