...
const button_pre = document.getElementById('button-presentnow')
...
button_pre.addEventListener('click', OnClickScreenSharing)
...
function OnClickScreenSharing() {
    const icon_present = document.getElementById('icon-present')
    if (icon_present.name === 'eye-outline') {
        icon_present.name = 'eye-off-outline'
        button_pre.style.background = 'red'
        button_pre.style.color = 'white'
        button_pre.style.borderColor = 'white'
        turnOnScreen()
    } else {
        icon_present.name = 'eye-outline'
        button_pre.style.background = 'white'
        button_pre.style.color = 'black'
        button_pre.style.borderColor = 'darkgray'
        turnOffScreen()
    }
...
function turnOnScreen() {
    ...
    navigator.mediaDevices.getDisplayMedia(option)
        .then(function(streams) {
            // const track = localStream.getVideoTracks()[0]
            // track = streams mytabvideo
            for (let socket_id in peers) {
                for (let index in peers[socket_id].streams[0].getTracks()) {
                    for (let index2 in streams.getTracks()) {
                        if (peers[socket_id].streams[0].getTracks()[index].kind === streams.getTracks()[index2].kind) {
                            peers[socket_id].replaceTrack(peers[socket_id].streams[0].getTracks()[index], streams.getTracks()[index2], peers[socket_id].streams[0])
                            break;
                        }
                    }
                }
            ...
            // socket.emit('signal', localStream)
        }, function(error) {
